...
@author: 
    ...
    Code to compile some data for GF week.
...
# %% Swifts CP to goal rate

#Read in team stats for this year
teamStats_2023 = collatestats.getSeasonStats(baseDir = baseDir,
                                             years = [2023],
                                             fileStem = 'teamStats',
                                             matchOptions = ['regular', 'final'],
                                             joined = True, addSquadNames = True)

#Extract Swifts CP to goal rate for the year
swiftsCP2G_2023 = teamStats_2023.loc[teamStats_2023['squadName'] == 'Swifts',
                                     ][['matchId', 'roundNo', 'squadName', 'oppSquadName', 'centrePassToGoalPerc']]

# %% Teams of 10

#Start with this year
teamsOf10_2023 = {'matchId': [], 'roundNo': [], 'squadName': []}
...
#Read in player lists for the year
playerLists_2023 = collatestats.getSeasonStats(baseDir = baseDir,
                                               years = [2023],
                                               fileStem = 'playerList',
                                               matchOptions = ['regular', 'final'],
                                               joined = True, addSquadNames = True)
...
for matchId in playerStats_2023['matchId'].unique():
    
    #Get the two squads for the current match
    currMatchPlayers = playerLists_2023.loc[playerLists_2023['matchId'] == matchId,].reset_index(drop = True)
    
    #Get the two squad names
    currSquadIds = currMatchPlayers['squadId'].unique()
    
    #Loop through the two squad Id's
    for squadId in currSquadIds:
        
        #Get the player list for the current squad
        squadPlayerList = list(currMatchPlayers.loc[currMatchPlayers['squadId'] == squadId,
                                                    ]['playerId'])
        
        #Collate minutes played across all players
        squadMinutes = [playerStats_2023.loc[(playerStats_2023['matchId'] == matchId) & (playerStats_2023['playerId'] == playerId)]['minutesPlayed'].values[0] for playerId in squadPlayerList]
        
        #Check for any zero minute players in list
        if not any(ii == 0 for ii in squadMinutes):
            
            #Add to the dataset
            teamsOf10_2023['matchId'].append(matchId)
            teamsOf10_2023['roundNo'].append(currMatchPlayers['roundNo'].unique()[0])
            teamsOf10_2023['squadName'].append(squadDict[squadId])
            
#Convert to dataframe
teamsOf10_2023_df = pd.DataFrame.from_dict(teamsOf10_2023)

#Get a count to see who has done it the most
teamsOf10_2023_count = teamsOf10_2023_df.groupby('squadName').count()

#Read in finals data over time
playerStats_finals = collatestats.getSeasonStats(baseDir = baseDir,
                                                 years = 'all',
                                                 fileStem = 'playerStats',
                                                 matchOptions = ['final'],
                                                 joined = True, addSquadNames = True)
playerLists_finals = collatestats.getSeasonStats(baseDir = baseDir,
                                                 years = 'all',
                                                 fileStem = 'playerList',
                                                 matchOptions = ['final'],
                                                 joined = True, addSquadNames = True)

#For each year, identify the grand final match Id
gfMatchId = {year: [] for year in playerStats_finals['year'].unique()}
for year in gfMatchId.keys():
    #Identify the maximum round
    maxRound = playerStats_finals.loc[playerStats_finals['year'] == year,]['roundNo'].max()
    #Deal with max round being 1
    #The final game will therefore be the grand final
    if maxRound == 1:
        #Get the maximum game number
        maxGame = playerStats_finals.loc[playerStats_finals['year'] == year,]['gameNo'].max()
        #Get the matchId for this game
        gfMatchId[year] = playerStats_finals.loc[(playerStats_finals['year'] == year) &
                                                 (playerStats_finals['gameNo'] == maxGame),
                                                 ]['matchId'].unique()[0]
    else:
        #Get the match Id for the max round
        gfMatchId[year] = playerStats_finals.loc[(playerStats_finals['year'] == year) &
                                                 (playerStats_finals['roundNo'] == maxRound),
                                                 ]['matchId'].unique()[0]
        
#Record the details for when entire team was used
teamsOf10_gf = {'matchId': [], 'year': [], 'squadName': []}

#Loop through years
for year in gfMatchId.keys():
    
    #Get match Id
    matchId = gfMatchId[year]
    
    #Get the two squads for the current match
    currMatchPlayers = playerLists_finals.loc[playerLists_finals['matchId'] == matchId,].reset_index(drop = True)
    ...
    for squadId in currSquadIds:
        ...
        #Collate minutes played across all players
        squadMinutes = [playerStats_finals.loc[(playerStats_finals['matchId'] == matchId) & (playerStats_finals['playerId'] == playerId)]['minutesPlayed'].values[0] for playerId in squadPlayerList]
        
        #Check for any zero minute players in list
        if not any(ii == 0 for ii in squadMinutes):
            
            #Add to the dataset
            teamsOf10_gf['matchId'].append(matchId)
            teamsOf10_gf['year'].append(year)
            teamsOf10_gf['squadName'].append(squadDict[squadId])

#Convert to dataframe
teamsOf10_gf_df = pd.DataFrame.from_dict(teamsOf10_gf)

# %% Hadley & Proud in wins vs. losses

#Read in player stats for this year
playerStats_2023 = collatestats.getSeasonStats(baseDir = baseDir,
                                               years = [2023],
                                               fileStem = 'playerStats',
                                               matchOptions = ['regular', 'final'],
...
#Extract Swifts player stats across the year
swiftsPlayerStats_2023 = playerStats_2023.loc[playerStats_2023['squadName'] == 'Swifts',].reset_index(drop = True)

#Loop through entries and identify if it was a win or loss
matchResult = []
for ii in swiftsPlayerStats_2023.index:
    
    #Grab match Id
    matchId = swiftsPlayerStats_2023.iloc[ii]['matchId']
    
    #Identify Swifts scoring vs. opposition scoring in team stats
    swiftsScore = teamStats_2023.loc[(teamStats_2023['matchId'] == matchId) &
                                     (teamStats_2023['squadName'] == 'Swifts'),
                                     ]['points'].values[0]
    oppScore = teamStats_2023.loc[(teamStats_2023['matchId'] == matchId) &
                                  (teamStats_2023['squadName'] != 'Swifts'),
                                  ]['points'].values[0]
    
    #Append as win, loss or draw
...
#Append to dataframe
swiftsPlayerStats_2023['matchResult'] = matchResult

#Set list for players to extract
keySwiftsPlayers = [80439, 80574]

#Extract key player stats
keySwiftsPlayerStats_2023 = swiftsPlayerStats_2023.loc[swiftsPlayerStats_2023['playerId'].isin(keySwiftsPlayers)]

#Group by wins vs. losses and average player stats
swiftsPlayerStats_byResult = keySwiftsPlayerStats_2023.groupby(['playerId','matchResult']).mean(numeric_only = True)

# %% Super Shot makes

#Collate super shot makes per match from the season
superShotMakes = teamStats_2023.groupby(['matchId','roundNo','gameNo']).sum(numeric_only = True)['goal2']
