...
@author: 
    ...
    who are selectable for the 2023 Netball Scoop fantasy season.
...
for squadId in list(squadDict.keys()):
    ...
    teamOppStats = pd.concat([playerStats[2021].loc[playerStats[2021]['oppSquadId'] == squadId, ],
                              playerStats[2022].loc[playerStats[2022]['oppSquadId'] == squadId, ]]
                             ).reset_index(drop = True)
    
    #Extract the stats not against the current team
    leagueOppStats = pd.concat([playerStats[2021].loc[playerStats[2021]['oppSquadId'] != squadId, ],
                                playerStats[2022].loc[playerStats[2022]['oppSquadId'] != squadId, ]]
                               ).reset_index(drop = True)
...
# %% Make predictions of player stats for entire 2023

"""

The first step in developing the bot is to create a prediction of what's going
to happen across the entire 2023 season. This serves as the foundation for selecting
...
also the preseason stats. We then use a scaling balance to predict the actual
players season fantasy score based on these two settings.
...
for playerId in fantasyPlayerDetails['playerId']:
    ...
    for roundNo in roundList:
        ...
        if len(selectPlayerPreseasonStats) > 0:
             ...
             #Set the weights for matches. For preseason stats only we simply
             #use a weight of 1 for each of the matches.
             
             #Set the default weights        
             weights = np.ones(len(selectPlayerPreseasonStats))
    ...
    #Season based stats
    predictedPlayerData_season.to_csv(f'..\\data\\fantasyBot2023\\statPredictionsFullSeason\\{playerId}_season.csv',
                                      index = False)
    #Preseason based stats
    predictedPlayerData_preseason.to_csv(f'..\\data\\fantasyBot2023\\statPredictionsFullSeason\\{playerId}_preseason.csv',
                                         index = False)
    
    #Print confirmation
    print(f'Exported data for {playerName} (id: {playerId})...')

# %% ----- End of statPredictions2023_fullSeason.py ----- %% #